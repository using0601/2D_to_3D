...
We provide a converter script ```convert.py```, to extract SfM information from input images. To use them, please first install a recent version of COLMAP (ideally CUDA-powered.) Put the images you want to use in a directory ```<location>/input```.
```
<location>
|---input
    |---<image 0>
    ...
    |---...
```
 If you have COLMAP on your system path, you can simply run 
```shell
python convert.py -s <location>
```
Alternatively, you can use the optional parameter ```--colmap_executable``` to point to the COLMAP executable. Please note that on Windows, the executable should point to the COLMAP ```.bat``` file that takes care of setting the execution environment. Once done, ```<location>``` will contain the expected COLMAP data set structure in addition to some temporary data.
